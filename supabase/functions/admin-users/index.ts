...
const authEmail = (username: string) => {
  /* admin กรอกอีเมลจริงของคนขับเป็นชื่อผู้ใช้ได้ (เช่น VPPW002@gmail.com) ซึ่งเป็นสิ่งที่
     คนตั้งบัญชีคาดว่าจะใช้ล็อกอิน ของเดิมแทน @ ด้วย _ แล้วต่อ @tms.local ต่อท้าย
     ได้ vppw002_gmail.com@tms.local ที่ไม่มีใครเดาถูกและพิมพ์ตามหน้าจอแล้วเข้าไม่ได้
     ถ้าเป็นอีเมลอยู่แล้ว ใช้ตามนั้น ที่เหลือค่อยแปลงเป็นชื่อในโดเมนภายในเหมือนเดิม */
  const clean = username.trim().toLowerCase()
  if (/^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$/.test(clean)) return clean
  return `${clean.replace(/[^a-z0-9._-]/g, '_')}@tms.local`
}
