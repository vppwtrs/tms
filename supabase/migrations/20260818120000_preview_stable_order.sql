-- ลำดับของเที่ยวในหน้า "เที่ยวจาก TMS" ไม่คงที่
--
-- เรียงด้วย trip_no อย่างเดียว แต่ TMS ใช้เลขเที่ยวซ้ำกันได้ข้ามคลัง
-- (เห็นสองแถวเลข 20260817003 พร้อมกันมาแล้ว) Postgres ไม่รับประกันลำดับ
-- ของแถวที่คีย์เรียงเท่ากัน สองแถวนั้นจึงสลับที่กันเองทุกครั้งที่โหลดหน้า
...
begin
  ...
  select json_build_object(
    ...
    'trips', coalesce((
      /* TMS ใช้เลขเที่ยวซ้ำกันได้ข้ามคลัง เรียงด้วยเลขเที่ยวอย่างเดียว
         ลำดับของสองแถวที่เลขเท่ากันจึงสลับไปมาทุกครั้งที่โหลด
         อ่านแล้วเหมือนข้อมูลมั่ว ทั้งที่เป็นคนละเที่ยวจริง ๆ */
      select json_agg(x order by x.trip_no, x.warehouse_code, x.license_plate)
        from (
          select t.tms_id, t.trip_no, t.status, t.status_id, t.reason,
                 ...
                 /* เลขใบจริง ไม่ใช่แค่จำนวน — เลข PL คือสิ่งที่คนคุยกับคลังและร้านค้าใช้อ้าง
                    เห็นแค่ "12 ใบ" แล้วต้องไปเปิด TMS อีกจอเพื่อรู้ว่าใบไหนบ้าง
                    จำกัด 6 ใบพอให้จำหน้าตาได้ ที่เหลือให้เปิดดูในหน้าออเดอร์ */
                 coalesce((
                   select array_agg(p.no order by p.no)
                     from (
                       select distinct s.picking_list_no as no
                         from public.tms_shipments s
                        where s.tms_trip_id = t.tms_id
                        order by 1
                        limit 6
