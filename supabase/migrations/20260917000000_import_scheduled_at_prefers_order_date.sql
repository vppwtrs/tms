-- scheduled_at เคยยึด tms_shipments.plan_delivery_date ก่อนเสมอ (coalesce(plan_delivery_date, trip_date))
-- แต่ plan_delivery_date จาก TMS เป็นวันที่วางแผนตอนสร้าง PL ซึ่งเพี้ยนได้ (เคยเจอค้างที่ 15 ก.ย.
-- ทั้งที่เที่ยวออกวิ่งจริงวันที่ 17) ทำให้ออเดอร์หลุดจากตัวกรอง "วันนี้" ของหน้าจัดการออเดอร์
-- เปลี่ยนให้ยึด tms_trips.order_date (ระดับเที่ยว ตรงกับวันออกวิ่งจริงจาก TMS) เป็นหลักก่อนเสมอ
-- ตกมาที่ trip_date/plan_delivery_date ระดับ PL เฉพาะตอน order_date ว่างเท่านั้น
...
-- backfill: แก้ scheduled_at ของออเดอร์ที่นำเข้าจาก TMS ให้ตรงกับ tms_trips.order_date
-- จำกัดเฉพาะออเดอร์ที่ผูกกับเที่ยว TMS จริง (ผ่าน trips.id = tms_trips.trip_id) และค่าปัจจุบันเพี้ยนไปจาก order_date เท่านั้น
update public.orders o
   set scheduled_at = t.order_date::timestamptz,
       updated_at = now()
  from public.trips tr
  join public.tms_trips t on t.trip_id = tr.id
 where o.trip_id = tr.id
   and t.order_date is not null
   and o.scheduled_at::date <> t.order_date;
