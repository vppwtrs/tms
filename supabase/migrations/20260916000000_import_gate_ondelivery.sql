-- ประตูนำเข้าเที่ยวจาก TMS: เดิมเปิดตั้งแต่ Confirm (status_id 2) เจ้าของงานสั่งขยับ
-- ขึ้นไปเป็น OnDelivery (status_id 4) — เที่ยวต้องออกวิ่งจริงแล้วที่ TMS ก่อนถึงจะ
-- สร้างเป็น order/ส่งถึงคนขับในระบบนี้ได้ แผนที่ยังอยู่แค่ Confirm/Handling ยังเปลี่ยนได้
-- ที่ TMS จึงยังไม่ถือเป็นของแน่นอนพอจะสั่งงาน
...
begin
  ...
  /* ประตูนำเข้า: TMS ต้องออกวิ่งแล้ว = OnDelivery หรือสถานะที่เดินหน้าไปจากนั้น
     (16 ก.ย. 2569 — ขยับจาก Confirm ขึ้นมาเป็น OnDelivery ตามที่เจ้าของงานสั่ง)
     เที่ยวที่ยัง Confirm/Handling คือแผนที่ยังเปลี่ยนได้ทั้งรถ คนขับ และรายการของ
     นำเข้าตอนนั้นเท่ากับสั่งงานคนขับด้วยข้อมูลที่ยังไม่นิ่ง

     ห้ามเทียบเท่ากับ 'ondelivery' ตรง ๆ: TMS เดินสถานะต่อเป็น Completed
     เที่ยวที่ออกวิ่งไปแล้วและจบงานแล้วจึงถูกปฏิเสธตลอดกาล ทั้งที่ผ่านด่านนี้มานานแล้ว
...
-- รายชื่อเที่ยว "พร้อมนำเข้า" ที่ป้อนให้ auto_import_trips ต้องขยับตามด่านบนให้ตรงกัน
-- ไม่งั้นรอบอัตโนมัติจะพยายามนำเข้าเที่ยวที่ยัง Confirm/Handling อยู่ แล้วชนด่านข้างบน
-- ทุกรอบ กลายเป็น "failed" เต็มจอทั้งที่ไม่ใช่ของพัง
  ...
  for v_t in
    select t.tms_id,
     ...
     where t.trip_id is null
       ...
       /* ออกวิ่งแล้ว (OnDelivery ขึ้นไป) — 16 ก.ย. 2569 ขยับจาก Confirm/Handling (2, 3)
          ตามที่เจ้าของงานสั่ง ต้องตรงกับด่านใน import_tms_trip เสมอ */
       and (
         case
           when t.status_id is not null then t.status_id between 4 and 5
           else lower(btrim(coalesce(t.status, ''))) in
             ('ondelivery', 'on delivery', 'delivering', 'delivered', 'complete', 'completed')
         end
       )
