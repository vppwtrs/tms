-- แก้ trip_rows.trip_no ในรายงานให้โชว์เลขทริปของ TMS บริษัท (เช่น 20260917005)
-- ไม่ใช่เลขภายในของเรา (TRP-2026-xxxx) — ฝ่ายบัญชี/ออฟฟิศเทียบเลขกับ TMS เป็นหลัก
-- ถ้าเที่ยวไหนไม่มี tms_trip_no (สร้างเองในระบบ ไม่ได้มาจาก TMS) ค่อย fallback ไปใช้ trip_no เดิม
...
begin
  ...
  tr as (
    select t.id,
           t.vehicle_id,
           t.driver_id,
           coalesce(t.tms_trip_no, t.trip_no) as trip_no,
           t.status,
           t.freight_cost,
