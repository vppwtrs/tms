-- นำเข้างานที่ยังไม่จบให้เป็น "วางแผน" เสมอ เพื่อให้ประตูรับงานมีความหมาย
--
-- เดิม map สถานะ TMS ตรง ๆ: OnDelivery (4) กลายเป็น in_progress ตั้งแต่วินาทีที่นำเข้า
-- ซึ่งเป็นอัตโนมัติทั้งเส้น งานจึงข้ามหัวคนขับไปตั้งแต่แถวแรก ไม่ต้องรอ sync ด้วยซ้ำ
-- ตอนนี้มีแต่เที่ยวที่คนขับกดรับ (accept_trip) หรือ TMS ปิดจบแล้ว ที่เดินหน้าต่อได้
...
begin
  ...
  if v_vehicle is null
    ...
    /* เทียบด้วยกุญแจทะเบียน ไม่ใช่ข้อความดิบ — TMS ส่งคันเดียวกันมาได้หลายรูป
       ("3ฒน5038" กับ "3ฒน5038-02") เทียบตรง ๆ แล้วจะสร้างรถซ้ำขึ้นมาใหม่ */
    select min(id)
      into v_vehicle
  ...
  /* งานที่ยังไม่จบ เข้าระบบเป็น "วางแผน" เสมอ ต่อให้ TMS บอกว่าออกวิ่งแล้ว
     เพราะการนำเข้าเป็นอัตโนมัติทั้งเส้น ถ้าปล่อยให้เข้ามาเป็น "กำลังวิ่ง" ได้
     งานจะข้ามหัวคนขับไปตั้งแต่แถวแรก ปุ่มรับงานไม่มีความหมาย
     เที่ยวที่ TMS ปิดไปแล้วเป็นคนละเรื่อง — เป็นการบันทึกประวัติ ไม่มีอะไรให้รับ */
  ...
  values
    (
      ...
      /* เที่ยวที่จบไปแล้วไม่ต้องรอใครรับ ไม่งั้นจะค้างในช่อง "รอคนขับรับ" ตลอดกาล */
