...
import type { Driver, Order, TripDetail, Vehicle } from '../types'
import { useToast } from '../context/ToastContext'
import { useAuth } from '../context/AuthContext'
import { Badge, Button, ConfirmDialog, EmptyState, ErrorBox, Field, Modal, PageHeader, SearchInput, Select, TableSkeleton, Textarea } from '../components/ui'
import { ORDER_STATUS_LABEL, ORDER_TONE, PRIORITY_LABEL, TRIP_STATUS_LABEL, TRIP_TONE, VEHICLE_TYPE_LABEL } from '../utils/constants'
...
export default function Dispatch(): React.JSX.Element {
  const { push } = useToast()
  const { user } = useAuth()
  const canEdit = user?.role !== 'viewer'

  const boardApi = useApi<{ planned: TripDetail[]; in_progress: TripDetail[] }>(() => api.get('/trips/board'), [])
  const pendingApi = useApi<Order[]>(() => api.get('/orders/pending-unassigned'), [])
  const vehiclesApi = useApi<Vehicle[]>(() => api.get('/vehicles/available'), [])
  const driversApi = useApi<Driver[]>(() => api.get('/drivers/available'), [])
  ...
  const [cancelTrip, setCancelTrip] = useState<TripDetail | null>(null)
  const [addTo, setAddTo] = useState<TripDetail | null>(null)
  const [addSelected, setAddSelected] = useState<Set<number>>(new Set())

  const pendingOrders = pendingApi.data ?? []
  const filteredPending = useMemo(
    () => pendingOrders.filter((o) => {
      const s = q.trim().toLowerCase()
      return !s || o.order_no.toLowerCase().includes(s) || o.destination.toLowerCase().includes(s) || (o.customer_name ?? '').toLowerCase().includes(s)
    }),
    [pendingOrders, q],
  )

  const refetchAll = (): void => {
    boardApi.refetch(); pendingApi.refetch(); vehiclesApi.refetch(); driversApi.refetch()
  }
  ...
  const selectedVehicle = vehiclesApi.data?.find((v) => v.id === Number(vehicleId))
  const capacityPct = selectedVehicle && selectedVehicle.capacity_kg > 0 ? (selectedWeight / selectedVehicle.capacity_kg) * 100 : 0
  ...
  const createTrip = async (): Promise<void> => {
    ...
    setCreating(true)
    try {
      const res = await api.post<{ warning?: string }>('/trips', {
      ...
      else push('success', 'สร้างเที่ยวขนส่งเรียบร้อย')
  ...
  const runAction = async (fn: () => Promise<unknown>, okMsg: string): Promise<void> => {
    try {
      await fn()
      push('success', okMsg)
      refetchAll()
  ...
  const confirmCancelTrip = async (): Promise<void> => {
    if (!cancelTrip) return
    setBusyId(cancelTrip.id)
    await runAction(() => api.post(`/trips/${cancelTrip.id}/cancel`), `ยกเลิกเที่ยว ${cancelTrip.trip_no} แล้ว — รถ/คนขับกลับว่าง`)
    setCancelTrip(null)
    setBusyId(null)
  }

  const openAddOrders = (trip: TripDetail): void => {
    setAddTo(trip)
  ...
  const submitAddOrders = async (): Promise<void> => {
    ...
    setBusyId(addTo.id)
    try {
      const res = await api.post<{ warning?: string }>(`/trips/${addTo.id}/orders`, { order_ids: [...addSelected] })
      if (res.warning) push('warning', res.warning)
      else push('success', 'เพิ่มออเดอร์เข้าทริปเรียบร้อย')
      setAddTo(null)
      refetchAll()
    } catch (e) {
      push('error', e instanceof Error ? e.message : 'เพิ่มออเดอร์ไม่สำเร็จ')
    } finally {
      setBusyId(null)
    }
  }

  const removeOrderFromTrip = async (trip: TripDetail, orderId: number): Promise<void> => {
    setBusyId(orderId)
    await runAction(() => api.delete(`/trips/${trip.id}/orders/${orderId}`), 'ถอนออเดอร์ออกจากเที่ยวแล้ว')
    setBusyId(null)
  }

  const loading = boardApi.loading || pendingApi.loading || vehiclesApi.loading || driversApi.loading
  const error = boardApi.error || pendingApi.error || vehiclesApi.error || driversApi.error

  if (error) return <ErrorBox message={error} onRetry={refetchAll} />

  const planned = boardApi.data?.planned ?? []
  const inProgress = boardApi.data?.in_progress ?? []
  ...
  return (
    <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 24, alignItems: 'start' }}>
                ...
                <table className="table">
                  ...
                  <tbody>
                    {filteredPending.map((o) => (
                      <tr key={o.id} style={{ cursor: 'pointer', background: selected.has(o.id) ? 'var(--accent-050)' : undefined }} onClick={() => toggleSelect(o.id)}>
                        <td>
                          <input
                            type="checkbox"
                        ...
                        <td>
                          {/* ป้าย "ด่วน" มาอยู่ข้างเลขที่ — ตัดคอลัมน์สถานะทิ้งทั้งคอลัมน์
                              (ปกติ = ไม่ต้องบอก ป้ายมีค่าเฉพาะตอนผิดปกติ) */}
                    ...
                    {filteredPending.length === 0 && (
                      <tr>
                        <td colSpan={5}>
                          <EmptyState icon={<IconBox size={30} />} title={q ? 'ไม่พบออเดอร์ที่ค้นหา' : 'ไม่มีออเดอร์รอจัดคิว'} desc={q ? '' : 'ออเดอร์ใหม่จะปรากฏที่นี่'} />
                        </td>
                      </tr>
                    )}
                  </tbody>
            ...
            <div>
              <div className="form-grid" style={{ gridTemplateColumns: '1fr' }}>
                <Field label="รถ (เฉพาะคันที่ว่าง)" required>
                  <Select value={vehicleId} onChange={(e) => setVehicleId(e.target.value)}>
                    ...
                    {vehicles.map((v) => (
                      <option key={v.id} value={v.id}>{v.plate_no} · {VEHICLE_TYPE_LABEL[v.vehicle_type]} ({fmtWeight(v.capacity_kg)})</option>
                    ))}
                  </Select>
                </Field>
                ...
                <Field label="หมายเหตุเที่ยว">
                  <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="เช่น ลำดับส่งสินค้า / จุดพักรถ" style={{ minHeight: 52 }} />
                </Field>
              </div>
              ...
              <div style={{ margin: '14px 0', padding: '10px 12px', background: 'var(--surface-2)', borderRadius: 10, border: '1px solid var(--line)' }}>
                ...
                {selectedOrders.length > 4 && <div className="text-xs text-muted" style={{ marginTop: 4 }}>และอีก {selectedOrders.length - 4} ใบ...</div>}
              ...
              <Button variant="accent" size="lg" style={{ width: '100%' }} icon={<IconPlus size={16} />} loading={creating} onClick={createTrip}>
                สร้างเที่ยวขนส่ง
              </Button>
            </div>
          </div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, alignItems: 'start' }}>
          ...
          <div>
            ...
            {planned.map((t) => (
              <TripCard
                ...
                busy={busyId === t.id}
                canEdit={canEdit}
                onStart={() => runAction(() => api.post(`/trips/${t.id}/start`), `เริ่มเที่ยว ${t.trip_no} — สถานะออเดอร์เป็นกำลังขนส่ง`)}
                onAddOrders={() => openAddOrders(t)}
                onCancel={() => setCancelTrip(t)}
                onRemoveOrder={(orderId) => removeOrderFromTrip(t, orderId)}
              />
            ))}
          </div>
          ...
          <div>
            ...
            {inProgress.length === 0 && <div className="card"><EmptyState title="ไม่มีเที่ยวกำลังขนส่ง" desc="เที่ยวที่เริ่มแล้วจะแสดงที่นี่แบบเรียลไทม์" /></div>}
            {inProgress.map((t) => (
              <TripCard
                key={t.id}
                trip={t}
                busy={busyId === t.id}
                canEdit={canEdit}
                onComplete={() => runAction(() => api.post(`/trips/${t.id}/complete`), `เที่ยว ${t.trip_no} เสร็จสิ้น — ออเดอร์เป็นส่งสำเร็จ`)}
                onCancel={() => setCancelTrip(t)}
              />
            ))}
          </div>
      ...
      {/* Modal เพิ่มออเดอร์ในเที่ยวที่วางแผนแล้ว */}
      <Modal
        open={addTo !== null}
        ...
        footer={
          <>
            ...
            <Button variant="primary" onClick={submitAddOrders} loading={busyId !== null} disabled={addSelected.size === 0}>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 380, overflowY: 'auto' }}>
          {pendingOrders.map((o) => (
            <label key={o.id} style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '8px 10px', borderRadius: 8, border: '1px solid var(--line)', cursor: 'pointer', background: addSelected.has(o.id) ? 'var(--primary-050)' : 'var(--surface)' }}>
              <input type="checkbox" checked={addSelected.has(o.id)} onChange={() => toggleSelect2(addSelected, setAddSelected, o.id)} style={{ accentColor: 'var(--accent)' }} />
              <div style={{ flex: 1 }}>
      ...
      <ConfirmDialog
        ...
        message={cancelTrip ? <>ต้องการยกเลิกเที่ยว <b>{cancelTrip.trip_no}</b> ({cancelTrip.vehicle_plate} · {cancelTrip.driver_name}) ใช่หรือไม่? ออเดอร์ในเที่ยวจะกลับเป็น <b>รอจัดคิว</b> และรถ/คนขับจะว่างทันที</> : ''}
...
function toggleSelect2(prev: Set<number>, setter: (s: Set<number>) => void, id: number): void {
  const next = new Set(prev)
  if (next.has(id)) next.delete(id)
  else next.add(id)
  setter(next)
  ...
  return (
    <div className="trip-card">
      ...
      <div className="trip-meta">
        <span><IconTruck size={14} style={{ verticalAlign: -2 }} /> <b>{trip.vehicle_plate}</b> · {VEHICLE_TYPE_LABEL[trip.vehicle_type as keyof typeof VEHICLE_TYPE_LABEL] ?? trip.vehicle_type}</span>
